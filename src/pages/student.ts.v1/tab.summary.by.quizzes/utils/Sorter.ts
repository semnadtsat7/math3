...
export function load ()
{
  ...
  const orderBy = ss.getItem('student-summary-by-quizzes.orderBy') || 'order';
  const order = ss.getItem('student-summary-by-quizzes.order') || 'asc';
...
export function set (orderBy: string, order: string)
{
  ...
  ss.setItem('student-summary-by-quizzes.orderBy', orderBy);
  ss.setItem('student-summary-by-quizzes.order', order);
}
...
export function sort (quizzes: Quiz[], { orderBy, order }: Sort)
{
  ...
  else if (orderBy === 'best' || orderBy === 'play' || orderBy === 'hint' || orderBy === 'help' || orderBy === 'usageAvg')
  {
    if (order === 'asc')
    {
