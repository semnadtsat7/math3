...
class Page extends React.Component
{
    ...
    async fetch()
    {
        ...
        return ref.onSnapshot (
            snapshot =>
            {
                ...
                {
                    const id = doc.id
                    const { map, order, quizName } = doc.data()

                    return { id, map, order, quizName }
                })
    ...
    async handleCreate()
    {
        ...
        try
        {
            const res = await Firebase.functions().httpsCallable('quizzes-quiz-create')({
                "quizName": quizName,
                "description": '',
                "map": 0,
                "isActive": false,
            ...
            this.props.history.push('/quiz-lesson/' + res.data);
        }
        catch (err)
        {
            console.log (err)
